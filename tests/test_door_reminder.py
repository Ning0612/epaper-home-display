...
def _make_current(temp: float = 22.0, feels_like: float | None = None) -> dict:
    return {"main": {"temp": temp, "feels_like": feels_like if feels_like is not None else temp}}
...
def test_clear_weather_no_reminder():
    current = _make_current(22.0)
    forecast = [_make_forecast(800, 22.0, 0.0)]
    assert generate_door_exit_text(current, forecast) is None
...
def test_pop_below_threshold_no_umbrella():
    forecast = [_make_forecast(800, 22.0, pop=0.5)]
    result = generate_door_exit_text(_make_current(22.0), forecast)
...
def test_warm_feels_like_no_jacket():
    ...
    assert generate_door_exit_text(current, []) is None
...
def test_max_two_conditions_output():
    """Rain + cold both triggered; temp-drop is bypassed by cold elif branch. Output has at most 2 items."""
    current = _make_current(temp=10.0, feels_like=8.0)
    forecast = [_make_forecast(500, temp=3.0, pop=0.9)]
    result = generate_door_exit_text(current, forecast)
    assert result is not None
    # Exactly rain + cold — third condition (temp drop) is suppressed by elif
    assert "雨傘" in result
    assert "外套" in result
    # "防曬" must not appear (hot condition not met)
...
def test_forecast_window_limited_to_4():
    """Rain only in slot index 5 (beyond window) must not trigger."""
    current = _make_current(22.0)
