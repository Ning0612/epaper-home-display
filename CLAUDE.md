...
- Dirty-region partial refresh (`epd7in5_V2` only): on non-full-refresh writes, `RealEpaper` (`app/display/epaper.py`) diffs against the last frame it wrote via `app/display/dirty_region.compute_dirty_regions()` and sends only the changed rectangle(s) through `init_part()`/`display_Partial()`; a no-op diff skips the panel write entirely. Falls through to the existing init_fast/full path (so `epd7in3e` is unaffected) whenever there's no baseline yet, the image isn't the panel's native size, or the driver lacks the partial-refresh API. `pack_mono_buffer()` inverts bits like `getbuffer()` does — hardware-verified on a real panel after a source-only trace gave the opposite (wrong) answer, so don't re-derive this convention from `display_Partial()`'s internals without a hardware check. Full algorithm detail (tile size, `MAX_REGIONS` fallback, 8-byte X alignment): see [docs/architecture.md](docs/architecture.md).
...
printer.creds_path / printer.serial
```
