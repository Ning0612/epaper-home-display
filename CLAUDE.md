...
```
Bambu Lab cloud MQTT (mqtts://us.mqtt.bambulab.com:8883) → app/services/printer_mqtt.py (paho-mqtt + TLS, background thread)
  → app/logic/printer.py (parse_print_status, pure) → app/state.py (printer_*) → renderer_cards.py::_draw_card_water_printer()
```
`BambuMQTTService.start()`/`.stop()` are wired in `app/main.py` next to `MQTTService`, same background-thread reasoning (not itself a gathered coroutine). Connects to Bambu's cloud MQTT broker with standard TLS certificate verification, using credentials from `printer.creds_path` (`data/bambu_creds.json` by default): username `u_{uid}`, password `access_token`, and serial from either `printer.serial` or the credentials file. This is independent of the Pi's Mosquitto broker used for HydraCup and does not require LAN Only Mode. Unlike HydraCup, on connect it actively publishes a `pushall` request and merges incoming fields per-field rather than replacing the whole status (Bambu's report stream is incremental — a message missing a field must not blank out the last known value). No heartbeat-timeout staleness — offline is `printer_broker_connected` only. Missing or invalid `access_token`/`uid`/effective serial disables the integration with no connection attempts. Full protocol spec and auth setup flow: [docs/bambu-mqtt-protocol.md](docs/bambu-mqtt-protocol.md).
...
- Refresh cadence: the 1st `epaper.display()` call is a full refresh, then every `full_refresh_every`-th (default 10) call after that is a full refresh again (init, clears ghosting); the rest use init_fast (partial). Counted per `display()` call, not per panel write that actually touched the panel — a dirty-region no-op (see below) still advances the cadence. Note: `epd7in3e` has no init_fast — every write is a full refresh.
- Dirty-region partial refresh (`epd7in5_V2` only): on a non-full-refresh write, `RealEpaper` (`app/display/epaper.py`) diffs the new frame against the last frame it successfully wrote (`self._last_image`) using `app/display/dirty_region.compute_dirty_regions()`, and sends only the changed rectangle(s) via `init_part()`/`display_Partial()` instead of the whole panel. No visual change → the panel write is skipped entirely. Changes are clustered into up to `MAX_REGIONS` (6) disjoint rectangles via 4-connected tile grouping (40px tiles); beyond that it falls back to one rectangle covering the whole diff. X bounds are rounded outward to multiples of 8 (the panel addresses columns by byte). This path is skipped (falls through to the existing init_fast/full path) whenever there's no baseline yet, the image isn't already sized to the panel's native `(width, height)` (`getbuffer()`'s 480x800 auto-rotate isn't replicated here), or the driver lacks `display_Partial()`/`init_part()` — so `epd7in3e` is entirely unaffected. `pack_mono_buffer()` inverts bits exactly like `getbuffer()` does (convert("1") then XOR 0xFF) — hardware-verified on a real epd7in5_V2 panel. A theoretical trace through `display_Partial()`'s internal `~Image[...]` step suggested it should NOT be pre-inverted, but that derivation was wrong: without this XOR, partially-refreshed regions rendered with black/white flipped relative to the rest of the screen on actual hardware. Trust the hardware result over the source trace here.
